...
public class Blueprint implements IBpInfoContainer
{
  ...
  public List<BpElement>    strictMaxes         = new ArrayList<BpElement> (); // every element which has a strict maximum of items to administer
  // Following List: the elements that are used in computing ability match
  public List<Strand>       strands             = new ArrayList<Strand> ();
  //public List<ReportingCategory> reportingCategories = new ArrayList<ReportingCategory>();   // the elements that are to be used : computing ability match
  ...
  public double 			abilityWeight = 1.0;  		// explicit ability weight; used : new adaptive alg
  public double 			itemWeight;     // weight of blueprint satisfaction relative to ability when pruning items from final itemgroup selected
  public int 				randomizerIndex;   			// use this to randomly select next itemgroup from final cset2
  public int 				randomizerInitialIndex;    	// use this to randomly select first itemgroup from cset2  
  public double             abilityOffset = ABILITY_OFFSET; // a spurious value to offset the ability estimate by
  ...
  public boolean 			releaseAll = true;  // on a given BpElement for preemptively pruned items when the pool has too few items for a min
  public boolean 			recycleAll = true;  // on a given BpElement for recycling items when the pool has too few items for a min
  ...
  public Blueprint copy ()
  {
    ...
    BpElement newelem;
    bp.Initialize (segmentKey, segmentPosition, minOpItems, maxOpItems, bpWeight, itemWeight, abilityOffset, randomizerIndex, randomizerInitialIndex,
        info, startAbility, cSet1Size, cset1Order, slope, intercept, adaptiveVersion);
  ...
  public void Initialize (String segmentkey, int segmentPosition, int minOpItems, int maxOpItems, double bpWeight, double itemWeight, double abilityOffset
      , int randomizer, int initialRandom, int numAdministered, double startInfo, double startAbility
      , int lastAbilityPosition)
  {
    ...
    this.info = startInfo;
    this.startAbility = this.theta = startAbility;
  }
  ...
  public void Initialize (String segmentkey, int segmentPosition, int minOpItems, int maxOpItems
      , double bpWeight, double itemWeight, double abilityOffset
      , int randomizer, int initialRandom, double startInfo, double startAbility
      , int cset1Size, String cset1Order, double slope, double intercept)
  {
    this.segmentKey = segmentkey;
    this.segmentPosition = segmentPosition;
    this.minOpItems = minOpItems;
    ...
    this.randomizerIndex = randomizer;
    this.randomizerInitialIndex = initialRandom;
    this.startInfo = this.info = startInfo;
    this.startAbility = this.theta = startAbility;
  ...
  // / <summary>
  // / For each strict max element where numadministered >= maxitems, prune all
  // items
  // / </summary>
  public void pruneStrictMaxes ()
  {
    ...
    if (poolcount + numAdministered >= minOpItems)
      return;
    // unprune items until there are enough items to complete test or all items
  ...
  public void recycleItems (PriorAdmins admins)
  {
    ...
    {
      // Start below the strands to be more discriminating (assuming there is
    ...
    // starting from first chronological set of items used, arbitrarily release
  ...
  {
    this.startAbility = this.theta = val;
  ...
  public void AddStrand (Strand strand)
  {
    ...
    strand.vectorIndex = this.strands.size (); // see the statistical vectors below
                                          // that are 1-based indexing

    if (strand.isStrictMax == true) // usually also go here for quick retrieval
      this.strictMaxes.add (strand);
  }

  // / <summary>
  // / Get a vector of the bp elements that have strict maxes for use in pruning
  // item group
  // / </summary>
  ...
  // / <summary>
  // / Update the examinee-specific blueprint for contentlevel satisfaction and
  // ability
  // / </summary>
  ...
  public void ProcessResponse (ItemResponse resp, int segmentPosition)
  {
    ...
    if (!resp.isFieldTest && resp.score > -1) // is this operational item response
                                       // scored? Then use it to update ability
//        UpdateAbility (resp.itemPosition, resp.get_strand (), resp.get_b (), resp.score);
    	UpdateAbility (resp);
    if (!resp.isFieldTest && resp.segmentPosition == segmentPosition) // is this operational item on
                                               // this segment? Then update
                                               // blueprint satisfaction
      UpdateSatisfaction (resp.getBaseItem ());
  }
  /// <summary>
  /// Use this to incrementally update the ability estimates at the overall test level or at the strand level
  /// There is no lambda value for overall ability
  /// </summary>
  /// <param name="itemB"></param>
  /// <param name="itemScore">Make sure to send the 'compressed score' which converts partial credit to [0,1] for use in the AA Rasch model</param>
  private void UpdateAbility(ItemResponse response)
  {
      ...
      this.lastAbilityPosition = Math.max(lastAbilityPosition, response.itemPosition);
  }
...
//    ++numAdministered; // this item was administered on this test
    for (String cl : item.getContentLevels ())
    {
      ...
      // else this may not be an error in a segmented test except for the fact
...
//  private void UpdateAbility (int position, String strand, double itemB, int itemScore)
//  {
//
//    Strand s = getStrand (strand);
//    // NOTE: PRevious item responses on different segments may not have strands
//    // on this segment
//    // In these cases we just want to update the overall ability and leave the
//    // strands alone
//    // if (s == null)
...
//  }
  ...
  public double[] getLambdaVector ()
  {
    Strand strnd;
    double[] vals = new double[getNumStrands () + 1];
    // vals[0] = this.vals; there is NO test-level lambda so position zero is
    // undefined
    for (int i = 0; i < strands.size (); ++i)
    {
      strnd =  strands.get (i);
  ...
  public double[] getCutscoreVector ()
  {
    Strand strnd;
    double[] vals = new double[getNumStrands () + 1];
    // vals[0] = this.vals; there is no relevant test-level cutscore so position
    // zero is undefined
    for (int i = 0; i < strands.size (); ++i)
    {
      strnd = strands.get (i);
  ...
  //
  public Double             bpWeight = 1D;      	// (w2) weight of blueprint satisfaction metric relative to ability match metric (which is always 1.0). Weight associated with matching blueprint spec (w2)
  //================================Adaptive2 Algorithm============
  // 
  // new for 2013
  public Double 			rcAbilityWeight = 1D ;    // (w1) abliity weight WRT the reporting categories. Weight associated with matching reporting category information (w1)
  ...
  /**
	 ...
	 */
	public Double hweightBeforeOverallTargetMet = 0.2D; // = precisionTargetNotMetWeight
	
	/**
	 * H0 function weight after target is met (b)
	 */
	public Double hweightAfterOverallTargetMet = 0.1D; // = precisionTargetMetWeight
  ...
  /**
   * The number of standard errors below which the difference is considered “too close” 
   * to the adaptive cut to proceed.  In general, this will signal proceeding to a final
   * segment that contains off-grade items.
   */
  ...
  /**
   * Flag to indicate whether to terminate based on achieving required minimum in
   * reporting categories and overall
   */
  ...
  /**
   * Flag to indicate whether to terminate based on achieving required information
  ...
  /**
   * Terminate if achieved a score insufficiently distant from a specified score with sufficient precision 
   */
  public Boolean terminateBasedOnScoreTooClose = false;
  //public boolean terminationTooClose;
  /**
   * Flag to indicate whether to use AND or OR with the above conditions when they
   * are set true to decide the termination
   */
  ...
  {
      return _bluePrintElements.containsKey(ID) ? 
      		_bluePrintElements.get(ID) : getReportingCategory(ID);
  }
  ...
  {
  	ArrayList<BpElement> elements = new ArrayList<BpElement>(_reportingCategories.values());
  ...
  {
      return _reportingCategories.containsKey(ID) ? _reportingCategories.get(ID) : null;
  }    
  ...
  public ArrayList<ReportingCategory> getReportingCategories()
  {
	...
	public void initializeOverallBluePrint(SingleDataResultSet res)
		...
		if (record != null) {
			...
			abilityWeight							= (overallInformationMatchWeight != null)? overallInformationMatchWeight: abilityWeight;
			
			hweightBeforeOverallTargetMet 			= float2Double(record, "precisionTargetNotMetWeight");
			hweightAfterOverallTargetMet 			= float2Double(record, "precisionTargetMetWeight");
   ...
   * @throws SQLException
   */
  public void initializeBluePrintConstraints(SingleDataResultSet res) throws SQLException
  {        
...
public void dumpBP()// for old algorithm
{
  String shift = "         ";
	...
	_logger.info(shift + "randomizerInitialIndex: " + randomizerInitialIndex);  
